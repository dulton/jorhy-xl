...
typedef std::queue<DVRInfo> DvrInfoQueue;
...
typedef std::queue<GetLogInfoResp> LogInfoQueue;
...
/// 报警信息回复  
typedef AlarmInfo AlarmInfoResp, *LPAlarmInfoResp;
typedef std::queue<AlarmInfo> AlarmInfoQueue;
...
/// 停止报警数据回复  
typedef struct _stopAlarmInfoResp
{
	char ret;			///< 0-0xFF的异常信息码	
} StopAlarmInfoResp, *LPStopAlarmInfoResp;
